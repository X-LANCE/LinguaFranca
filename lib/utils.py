...
def load_ffn_weight(filename, template, layer, model):

    if model==None:
        model = AutoModelForCausalLM.from_pretrained(filename, device_map="balanced_low_0")

    component_parts = template.format(layer).split('.')
    obj = model
    for part in component_parts[1:]:
    ...
    component = obj.weight.detach().cpu().numpy()
    del model
    model = None
...
class ModelConfig:
    ...
    def __init__(self, filename, folder, split_num, load_model=False):
        self.filename = filename
        self.folder = folder
        self.split_num = split_num
        if load_model:
            self.model = AutoModelForCausalLM.from_pretrained(filename, device_map="auto")
        else:
            self.model = None
...
class LayerSplit:
    ...
    def load_param(self):
        self.ffn_weight = load_ffn_weight(self.config.filename, self.template, self.layer, self.config.model)

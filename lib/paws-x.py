...
class PAWSX(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, filepath, files):
        ...
        for path, f in files:
            for path2, f2 in files:
                # print(path[-8:], path2[-8:])
                # print(path == filepath)
                if path == filepath and (("en" in path2) and path[-8:]==path2[-8:]):
                    print(path, path2)
                    lines = (line.decode("utf-8") for line in f)
                    lines2 = (line.decode("utf-8") for line in f2)
                    data = csv.DictReader(lines, delimiter="\t", quoting=csv.QUOTE_NONE)
                    data2 = csv.DictReader(lines2, delimiter="\t", quoting=csv.QUOTE_NONE)

                    for id_, (row, row2) in enumerate(zip(data, data2)):
                        yield id_, {
                            "id": row["id"],
                            "sentence1": row["sentence1"],
                            "sentence2": row2["sentence2"],
                            "label": row["label"],
                        }
                    break
